<template>
  <div>
    <p class="mb-4 text-sm text-gray-500 dark:text-gray-400">
      基于官方历史平衡报告（<span class="font-mono">historical_balance</span>，经 k 值平滑），按周采样。展示各角色胜率随时间（跨版本）的演化。
    ...
    <p class="text-xs text-gray-400 dark:text-gray-500 leading-relaxed">
      说明：数据取自官方历史平衡报告（historical_balance），每个时点的胜率为该角色所在阵营取胜的占比、并经官方 k 值平滑；按 ISO 周采样（每周末次快照）。
      游戏内补丁无独立版本号，故时间轴以日期表示。样本不足 30 场的角色已置灰。
      如需按任意时间段或区分国服 / 外服，请用「时段排行」标签。
    </p>
  </div>
</template>
