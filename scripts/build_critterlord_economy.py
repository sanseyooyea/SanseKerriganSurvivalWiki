...
TimeScale 对产矿节奏的精确加成属引擎行为，不烘焙进 income/每秒 头条数字，仅在
incomeModel/notes 里如实登记原始 TimeScale 值。
...
def build_entry():
    ...
    for b in hives:
        ...
        notes = (f'唯一可直接建造的主巢。' if b['tier'] == 1
                 else '由上一档升级而来。')
        notes += f'每 {_fmt(per)}s 产出 {_fmt(amt)} 晶矿。'
        if ts:
            notes += f'自带永久运作加速 ×{_fmt(ts)}（TimeScale，提升自身运作/产矿节奏，无气耗）。'
        if b['gas']:
    ...
    return {
        ...
        'incomeModel': (
            ...
            '不能跨级直建；升级按「目标造价 − 当前造价」扣费。每档每 2.5 秒产出等于档位数的'
            '晶矿（主巢 32 = 每 2.5 秒 32 矿）。高档主巢需要气体建造（主巢 32 需 20 气）。'
            '\n\n'
            '【自带永久加速】主巢 2 及以上各自带永久运作加速（TimeScale：主巢 2 ×1.1、4 ×1.2、'
            '8 ×1.3、16 ×1.35、32 ×1.4），提升该建筑自身运作速度（含产矿节奏），越高档越快，'
            '无需操作、无气耗——故实际产矿快于表中「每 2.5s」的基础节奏。'
            '\n\n'
